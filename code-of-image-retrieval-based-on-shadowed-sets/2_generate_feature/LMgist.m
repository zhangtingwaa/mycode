...
if isnumeric(D)
    ...
    Nscenes = size(D,4);
    typeD = 3;
    if ~isfield(param, 'imageSize')%如果param不包含字段'imageSize'
...
if nargin<3
    ...
    if ~isfield(param, 'G')
        param.G = createGabor(param.orientationsPerScale, param.imageSize+2*param.boundaryExtension);%创建gabor滤波器！
    end
end
...
for i=1:Nscales
    for j=1:or(i)
        l=l+1;
        param(l,:)=[.35 .3/(1.85^(i-1)) 16*or(i)^2/32^2 pi/(or(i))*(j-1)];%创建gabor滤波器
    end
end
